...
struct mp_pool_s *mp_create_pool(size_t size) {
  ...
  p->head->last =
      (unsigned char *)p + sizeof(struct mp_pool_s) + sizeof(struct mp_node_s);
...
int main(int argc, char *argv[]) {
  ...
  printf("mp_align(123, 32): %d, mp_align(17, 32): %d\n", mp_align(24, 32),
         mp_align(17, 32));
  // printf("mp_align_ptr(p->current, 32): %lx, p->current: %lx,
  // mp_align(p->large, 32): %lx, p->large: %lx\n", mp_align_ptr(p->current,
  // 32), p->current, mp_align_ptr(p->large, 32), p->large);
